...
contract StructuredEther {
    ...
    function sellEther(uint amount, uint stake) public nonZero(amount) pctCap(stake) triggersUpdates {
        ...
        if (stake > 0) {
            ...
            accounts[owner][uint8(a.ETH)] = address(this).balance.sub(accounts[owner][uint8(a.stakedETH)]).sub(accounts[owner][uint8(a.totalOthersEther)]);
        }
    }
    ...
    function collectGlobalInterest(uint dateTo) internal {
        ...
        if (globInterest < accounts[owner][uint8(a.stakedETH)]) {
